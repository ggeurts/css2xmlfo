...

{
    ...
    public static String getCommonPrefix(String[] s)
    {
        if (s.length == 0)
        {
            return "";
    ...
    public static File getPackageStorage(Class c)
    {
    ...
    public static String getReSystemProperty(String name) throws IOException
    {
        InputStream in
                = Util.class.getClassLoader().
    ...
    public static String getSystemProperty(String propertyName) throws IOException
    {
    ...
    private static TimeZone getTimeZone(String s, int offset)
    {
        if (offset == -1)
        {
    ...
    private static int getTimeZoneOffset(String s)
    {
        int index;
        ...
        return s.charAt(s.length() - 1) == 'Z'
                ...
                : ((index = s.lastIndexOf("GMT")) != -1
                        ...
                        || (index = s.lastIndexOf('-')) != -1) && timeZonePattern.matcher(s.substring(index + 1)).matches()
                                ? index : -1));
    }

    public static boolean isDateSet(String dateTime)
    {
        return isDateSet(findFormat(dateTime));
    }

    private static boolean isDateSet(String[] format)
    {
    ...
    public static boolean isLanguageTag(String s)
    {
        ...
        return languages.contains(locale.getLanguage().toLowerCase())
                && ("".equals(locale.getCountry())
    ...
    public static boolean isTimeSet(long time)
    {
    ...
    public static boolean isTimeSet(String dateTime)
    {
        return isTimeSet(findFormat(dateTime));
    }

    private static boolean isTimeSet(String[] format)
    {
    ...
    public static String list(String[] list, String separator)
    {
    ...
    public static long parseDateTime(String s)
    {
        ...
        long time = parseTimestamp(s, new String[]
        {
            format[0]
        });
                    ...
                    if (s.length() > 16 && s.charAt(16) == ':')
                    {
                        ...
                        if (s.length() > 19 && s.charAt(19) == '.')
                        {
                            calendar.set(Calendar.MILLISECOND, 
                                    Math.round(Float.parseFloat("0." + s.substring(20, timeZoneOffset != -1 ? timeZoneOffset : s.length())) * 1000));
                        }
                    }
                }
            }
    ...
    public static long parseTimestamp(String s, String[] patterns)
    {
        Date result = null;
    ...
    public static String patternToRegexp(String pattern)
    {
        return pattern
                .replaceAll("\\+", "\\+")
                .replaceAll("\\.", "\\.")
    ...
    public static String[] readLineConfig(InputStream in) throws IOException
    {
        return readLineConfig(new BufferedReader(new InputStreamReader(in)));
    }

    public static String[] readLineConfig(BufferedReader in) throws IOException
    {
        String line = "";
    ...
    public static long setTime(long date, int hourOfDay, int minute, int second)
    {
        Calendar calendar = Calendar.getInstance();
