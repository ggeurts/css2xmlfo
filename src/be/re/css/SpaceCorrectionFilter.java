...
class SpaceCorrectionFilter extends XMLFilterImpl
{
    ...
    public void characters(char[] ch, int start, int length) throws SAXException
    {
        ...
        for (int i = 0; i < length; ++i)
        {
                    ...
                    && widths[ch[start + i] - 0x2000].length() > 0)
            {
                ...
                atts.addAttribute("", "leader-length", "leader-length", "CDATA", widths[ch[start + i] - 0x2000]);
    ...
    private static String[] loadWidths()
    {
        try
        {
            ...
            properties.load(SpaceCorrectionFilter.class.getResourceAsStream("res/space_correction.prop"));

            for (int i = 0; i < result.length; ++i)
            {
                String value = properties.getProperty(Integer.toString(0x2000 + i, 16).toLowerCase());
                if (value == null)
                {
                    value = properties.getProperty(Integer.toString(0x2000 + i, 16).toUpperCase());
                }
