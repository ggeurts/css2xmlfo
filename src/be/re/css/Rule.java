...
public class Rule
{
    ...
    private static String getElementName(Selector[] selectorChain, int position)
    {
        switch (selectorChain[position].getSelectorType())
        {
            case Selector.SAC_ELEMENT_NODE_SELECTOR:
                return ((ElementSelector) selectorChain[position]).
                        getLocalName();
    ...
    Property[] getProperties()
    {
        ...
        {
            cachedArray
                    = (Property[]) properties.values().toArray(new Property[properties.size()]);
        }
    ...
    /**
     * Splits this rule into a set of equivalent rules in which there is only
     * one property. For each property of this rule there will be a new one.
     */
    Rule[] split()
    {
