...
public class Property
{
  ...
  private static final String[]	SYSTEM_FONTS =
    ...
    public Property(String name, LexicalUnit value, boolean important, Map<String, String> prefixMap, URL baseUrl)
    {
        this.name = name;
        this.value = value;
        this.valueAsString = Util.lexicalUnitToString(value, !"font-family".equals(name), baseUrl);
        this.important = important;
        this.prefixMap = prefixMap;
    ...
    private boolean isFontSize(LexicalUnit unit)
    {
    ...
    private static boolean isFontWeight(String atom)
    {
        try
        {
            return Util.inArray(FONT_WEIGHT, atom)
    ...
    private Property[] splitBackground()
    {
        ...
        for (LexicalUnit i = value; i != null; i = i.getNextLexicalUnit())
        {
            ...
            else if (i.getLexicalUnitType() == LexicalUnit.SAC_RGBCOLOR || Util.inArray(COLORS, atom))
            {
                result.add(copy("background-color", atom));
            } 
            else if (Util.inArray(BACKGROUND_ATTACHMENT, atom))
            {
                result.add(copy("background-attachment", atom));
            } 
            else if (Util.inArray(BACKGROUND_REPEAT, atom))
            {
    ...
    private Property[] splitBorder()
    {
        ...
        for (LexicalUnit i = value; i != null; i = i.getNextLexicalUnit())
        {
            ...
            if (i.getLexicalUnitType() == LexicalUnit.SAC_RGBCOLOR
            ...
            else if (Util.inArray(BORDER_STYLE, atom))
            {
    ...
    private Property[] splitBorderWidth()
    {
        ...
        result[values.length + 1] = copy("border-before-width.conditionality", "retain");
    ...
    private Property[] splitFont()
    {
        ...
        for (LexicalUnit i = value; i != null; i = i.getNextLexicalUnit())
        {
            ...
            else if (Util.inArray(FONT_STYLE, atom))
            {
                result.add(copy("font-style", atom));
            } 
            else if (Util.inArray(FONT_VARIANT, atom))
            {
                result.add(copy("font-variant", atom));
            } 
            else if (isFontWeight(atom))
            {
                result.add(copy("font-weight", atom));
            } 
            else if ((i.getPreviousLexicalUnit() == null || i.getPreviousLexicalUnit().getLexicalUnitType() != LexicalUnit.SAC_OPERATOR_SLASH) && isFontSize(i))
            {
                result.add(copy("font-size", atom));
            } 
            else if (i.getLexicalUnitType() == LexicalUnit.SAC_OPERATOR_SLASH)
            {
                i = i.getNextLexicalUnit();
                result.add(copy("line-height", Util.lexicalUnitAtomLower(i, baseUrl)));
            } 
            else
            {
                remaining = remaining == null
                        ? originalAtom 
                        : (remaining + " " + originalAtom);
            }
        }
    ...
    private Property[] splitFourWays(String name, boolean important)
    {
        ...
        if (units.length == 1)
        {
            ...
            return setAtoms(names, new String[]
            {
                atom, atom, atom, atom
            });
        }
    ...
    private Property[] splitListStyle()
    {
        ...
        for (LexicalUnit i = value; i != null; i = i.getNextLexicalUnit())
        {
            ...
            else if (i.getLexicalUnitType() == LexicalUnit.SAC_URI)
            {
                result.add(copy("list-style-image", atom));
            } 
            else if (Util.inArray(LIST_STYLE_POSITION, atom))
            {
                result.add(copy("list-style-position", atom));
            } 
            else if (Util.inArray(LIST_STYLE_TYPE, atom))
            {
                result.add(copy("list-style-type", atom));
            }
        }
